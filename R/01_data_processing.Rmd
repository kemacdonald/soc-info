...
# score the manipulation check
manip_check %<>% 
  mutate(manip_check_score = ifelse(music_check_response == manip_check_order_action,
                                    "correct", "incorrect")) %>% 
  group_by(id) %>% 
  mutate(pass_manip_check = ifelse(str_detect(manip_check_score, "incorrect"),
                                              FALSE,
                                              TRUE)) %>% 
  select(id, condition, pass_manip_check) %>% 
  unique()
```
...
d_final %<>%
  ...
  mutate(hypothesis_type = factor(hypothesis_type) %>% fct_rev(),
         slider_response_type_collapsed = factor(slider_response_type_collapsed) %>% fct_rev()) 
```
...
write_csv(d_final, path = here("/data/02_tidy_data", 
                               "goal_actions_ver2_tidy_pilotA.csv"))
```
