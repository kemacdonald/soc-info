...
```{r}
d %>% 
  group_by(hypothesis_type, condition, slider_response_type_collapsed) %>% 
  summarise(m = mean(slider_value_norm_2)) %>% 
  ggplot(aes(x = slider_response_type_collapsed, y = m, fill = condition)) +
  geom_bar(stat = "identity", width = 0.4, position = position_dodge()) +
  geom_hline(yintercept = 0.3, linetype = "dashed") +
  lims(y = c(0,1)) +
  scale_fill_grey() +
...
d %>% 
  ...
  unique() %>% 
  count(action_response_collapsed, condition) %>% 
  group_by(condition) %>% 
  mutate(prop = n / sum(n)) %>% 
