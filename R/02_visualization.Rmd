...
Now as a function of action response 

```{r}
plot_data <- d_beliefs_collapsed %>% 
  left_join(., select(d, id, action_response_collapsed) %>% unique(), by = "id") %>% 
  mutate(hypothesis_type = factor(hypothesis_type) %>% fct_rev(),
         slider_response_collapsed = factor(slider_response_collapsed) %>% fct_rev()) 
...
plot_data %>% 
  ...
  ggrepel::geom_label_repel(data = filter(plot_data, hypothesis_type == "posterior"),
                                          aes(label = id)) +
