...
```{r libraries}
library(binom)
library(knitr); library(magrittr); library(tidyverse); library(here); library(grid)
...
d_e2_temp <- fromJSON(here("model/BDA/e2/e2-actionData.json"))
...
data_mod <- rbind(postpred %>% mutate(source = "model"), d_e2_pred %>% mutate(source = "data"))
...
```{r}
sub_plots_e2 <- cowplot::plot_grid(e2_rt_plot, e2_ent_plot, phi, labels = c("B", "C", "D"), 
                                ncol = 1,
