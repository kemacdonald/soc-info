...
cowplot::plot_grid(action_plot_e1, sub_plots, labels = c("A"),
                   rel_widths = c(1, 0.9))
```
...
```{r}
ggsave("e1_plot.png", plot = ggplot2::last_plot(), width = 9,
...
cowplot::plot_grid(e2_action_plot, sub_plots_e2, labels = c("A"),
                   rel_widths = c(1, 0.85))
```

```{r}
ggsave("e2_plot.png", plot = ggplot2::last_plot(), width = 9,
       path = here::here("writing/cogsci2018/figs"), 
       scale = 1.2)
```
