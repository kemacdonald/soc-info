...
m_act_logit_e2 <- d_model_e2 %>% 
  filter(social_condition == "social") %>% 
  stan_glm(action_num ~ condition_fact, 
                        data = .,
                        family = binomial(link = "logit"),
                        prior = t_prior, prior_intercept = t_prior)
...
RT on decision trials as a function of condition and social context.
