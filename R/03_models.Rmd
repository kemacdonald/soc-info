...
d_model <- d %>%
  ...
  distinct(id, condition, action_response, action_trial_time) %>% 
...
```{r}
d_entropy <- d %>% 
  filter(manip_check_score >= 2, str_detect(experiment, pattern = "e2")) %>% 
  ...
  mutate(entropy_change = posterior - prior,
         ...
         condition_fact = factor(condition, 
                                 levels = c("nogoal", "presentation", "learning", 
                                            "performance")))
```
...
samples_entropy %>% 
  ...
  summarise(m = mean(param_est), 
            hdi_lower = quantile(param_est, probs = c(0.05)),
            hdi_upper = quantile(param_est, probs = c(0.95)))
```

## Save posterior samples 

```{r}
posteriors <- list(action_m = tidy_samples,
                   action_rt_m = samples_rt,
                   entropy_m = samples_entropy)
