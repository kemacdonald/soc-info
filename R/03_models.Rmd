...
d_model_e1 <- d %>%
  ...
  mutate(condition_fact = factor(condition, 
         ...
         action_num = ifelse(action_r_collap == "both", 1, 0))
```
...
```{r}
samples_entropy_e1 <- m_entropy_e1 %>% 
  as.data.frame() %>% 
  as_tibble() %>% 
  dplyr::rename(no_goal = `(Intercept)`,
                learning_beta = condition_factlearning,
...
d_model_e2 <- d %>%
  ...
  mutate(condition_fact = factor(condition, 
                                 levels = c("learning", "nogoal", "presentation",
                                            "performance")),
         action_response_fact = factor(action_response, 
                                       levels = c("button", "handle", "both")),
...
```{r action logit model}
t_prior <- student_t(df = 7, location = 0, scale = 2.5)
m_act_logit_e2 <- stan_glm(action_num ~ condition_fact * social_condition, 
                        data = d_model_e2,
                        family = binomial(link = "logit"),
                        prior = t_prior, prior_intercept = t_prior)
```
...
samples_logit_act_e2 <- m_act_logit_e2 %>% 
  ...
  dplyr::rename(learning = `(Intercept)`,
                no_goal_beta = condition_factnogoal,
                performance_beta = condition_factperformance,
                presentation_beta = condition_factpresentation,
                social_beta =  social_conditionsocial,
                int_ng_social = `condition_factnogoal:social_conditionsocial`,
                int_present_social = `condition_factpresentation:social_conditionsocial`,
                int_perf_social = `condition_factpresentation:social_conditionsocial`
                ) %>% 
  mutate(sample_id = 1:n()) %>% 
  gather(key = effect, value = param_est, -sample_id) %>% 
  mutate(eff_type = ifelse(str_detect(effect, "int"), "interaction", "main effect")) 
...
```{r}
posteriors <- list(action_logit_e1 = samples_logit_act_e1,
                   rt_e1 = samples_rt_e1,
                   ent_e1 = samples_entropy_e1,
                   action_logit_e2 = samples_logit_act_e2)
