...
exp = {
        ...
        actions_slide: function () {
            ...
            if (exp.attempt < 3) {
                ...
                if (exp.attempt == 1) {
                    ...
                    instructions_html = `<table align="center"> ${goal_text_html2} </table>`;
                ...
                for (i = 0; i < exp.rand_action_labels.length; i++) {
                    ...
                    // check if there is " and " handles the case of both actions
                    if (action_label.includes(" and ")) {
        ...
        actions_close: function (response) {
            ...
            // store the action choice
            if (exp.attempt == 1) {
                exp.action_response1 = response;

            } else {
                exp.action_response2 = response;
            ...
            if (response == "both" || (exp.attempt == 1 && outcome == "music") ||
                (exp.attempt == 2 && 
                 ((outcome == "music" && (response == exp.action_response1 || exp.action_response1 == "both")) || 
                  (outcome == "light" && response != exp.action_response1 && exp.action_response1 != "both")))) {
                // then play sound which also advances slide (a little hacky)
                exp.play_music('action_slide');
            } else {
                setTimeout(function () {
            ...
            if (typeof response == 'undefined') {
                ...
                if (response == "both") {
                    ...
                    response_outcome = "toyLight.jpeg"
                           ...
                           (exp.action_response1 != response && exp.action_response1 != "both")) || 
                           (outcome == "light" && 
                            (exp.action_response1 == response || exp.action_response1 == "both"))) {
                    answer_success_message = `<font color="green"><b>Look, you made the toy turn on the light!</b></font>`
                    response_outcome = "toyLight.jpeg"
