...
rs.l1.tidy <- data.frame(rs.l1) %>% 
  gather(key=action, value=prob, -phi_learning, -phi_perf, -phi_pres, -obsPres)
```

```{r}
rs.l1.tidy %<>%
  mutate(goal = factor(case_when(
    phi_learning > .6 & phi_perf < .4 & phi_pres < .4 ~ "learning",
    phi_learning < .4 & phi_perf > .6 & phi_pres < .4 ~ "performance",
    phi_learning < .4 & phi_perf < .4 & phi_pres > .6 ~ "presentation",
    TRUE ~ "NA"
  )))
...
fig.l1 <- ggplot(rs.l1.tidy %>%
  ...
  facet_grid(obsPres~goal, labeller=label_both)+
