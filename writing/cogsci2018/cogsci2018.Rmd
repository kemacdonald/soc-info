...
depending on goals that we highlighted. We were also interested to see what people would choose to do when no goal is specified and they are free to explore. Importantly, participants were limited to making only a single action, which meant the opportunity cost for the alternative action was at its highest. Thus, participants were asked to imagine that they needed to act on a toy with an uncertain causal mechanism, and we assigned participants to different goal conditions: learning goal (i.e. learn how the toy works); performance goal (e.g. make the toy play music); presentation goal (i.e. impress their boss); and no goal.
...
e2_contrasts_act <- d_models$action_logit_e2 %>% 
...
ms_act_e2 <- d_models$action_logit_e2 %>% 
  select(-eff_type) %>% 
  spread(effect, param_est) %>% 
  mutate(no_goal_soc = no_goal + social_beta,
         performance_no_soc = no_goal + performance_beta,
         performance_soc = no_goal + performance_beta + social_beta*int_perf_social,
         learning_no_soc = no_goal + learning_beta,
         learning_soc = no_goal + learning_beta + social_beta*int_learn_social,
         presentation_no_soc = no_goal + presentation_beta,
         presentation_soc = no_goal + presentation_beta + social_beta*int_present_social) %>% 
  select(sample_id, no_goal, no_goal_soc:presentation_soc) %>% 
  gather(key = condition, value = param_est, -sample_id) %>% 
  mutate(act_prob_scale = logit_to_prob(param_est)) %>% 
  group_by(condition) %>% 
  summarise(prop = mean(act_prob_scale),
...
We modeled action decisions using a logistic regression specified as \texttt{$action \sim goal\_condition * social\_context$} with the No-Goal and No-Social condition as the reference category. 

TODO: create two models -- (1) learn, no-goal, and performance that test interaction between goal and social context, and (2) social model that also includes presentation condition to get main effect of goal manipu

Participants' tendency to select a "single" action varied across conditions in the predicted pattern (see Panel A of Fig 2), with the highest proportion occuring in the Learning context ($M_{learn} =$ `r ms_act_e1$prop[1] * 100`%, [`r ms_act_e1$hdi_lower[1] * 100`%, `r ms_act_e1$hdi_upper[1] * 100`%]), followed by the No Goal context ($M_{noGoal} =$ `r ms_act_e1$prop[2] * 100`%, [`r ms_act_e1$hdi_lower[2] * 100`%, `r ms_act_e1$hdi_upper[2] * 100`%]), then Performance ($M_{perform} =$ `r ms_act_e1$prop[3] * 100`%, [`r ms_act_e1$hdi_lower[3] * 100`%, `r ms_act_e1$hdi_upper[3] * 100`%]), and the fewest single actions in the Presentation condtion ($M_{present} =$ `r ms_act_e1$prop[4] * 100`%, [`r ms_act_e1$hdi_lower[4] * 100`%, `r ms_act_e1$hdi_upper[4] * 100`%]). 

### Decision times:

```{r rt contrasts e2}
e2_contrasts_rt <- d_models$rt_e2 %>% 
  group_by(condition) %>% 
  summarise(rt = median(rt_sec_scale),
            hdi_lower = quantile(rt_sec_scale, probs = 0.025),
            hdi_upper = quantile(rt_sec_scale, probs = 0.975),
